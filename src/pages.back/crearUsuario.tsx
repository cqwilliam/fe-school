...
export default function CrearUsuario({ userId }: { userId?: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4">
      ...
      <label>DNI:</label>
      <input type="text" name="dni" value={userData.dni} onChange={handleChange} required />
      
      <label>Fecha de Nacimiento:</label>
      <input type="date" name="birth_date" value={userData.birth_date || ''} onChange={handleChange} />
      
      <label>URL de Foto:</label>
      <input type="url" name="photo_url" value={userData.photo_url || ''} onChange={handleChange} />
      ...
      <button type="submit">{userId ? 'Actualizar Usuario' : 'Crear Usuario'}</button>
    </form>
  );
}
