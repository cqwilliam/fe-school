...
const CourseMaterial = () => {
  const { courseMaterialId } = useParams(); // Asegúrate que este ID en la ruta sea `[courseMaterialId]`
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <p><strong>Sección ID:</strong> {material.section_id}</p>
      <p><strong>Publicado por (Usuario ID):</strong> {material.published_by}</p>
      ...
      <p><strong>Fecha de Publicación:</strong> {new Date(material.published_at).toLocaleString()}</p>
    </div>
  );
};
